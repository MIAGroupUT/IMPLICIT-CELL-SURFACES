...
def main_function(experiment_directory):
    ...
    # load latent vectors from .mat file
    learned_vectors = np.squeeze(np.asarray([sio.loadmat(experiment_directory + '/latent_vecs.mat')['lat_vecs']], dtype=np.float32))

    # randomize order of latent vectors
    np.random.shuffle(learned_vectors)

    random_noise = np.random.normal(0.0, 0.01, size=(num_sequences, latent_size)).astype(np.float32)
    
    learned_vectors = learned_vectors + random_noise
    
    print(np.min(learned_vectors), np.max(learned_vectors))
    
    print(learned_vectors.shape)
    
    num_frames_per_sequence_to_reconstruct = specs["ReconstructionFramesPerSequence"]
    reconstruction_dims = specs["ReconstructionDims"]
    
    # Reconstruction -----------------------------------------------------------
    print("Generating {} new frames per sequence with resolution {}".format(num_frames_per_sequence_to_reconstruct,reconstruction_dims))
    ...
    for sequence_id in range(learned_vectors.shape[0]):
        ...
        #latent = latent * mult_factor
        print('\nlat. vec. preview:', 
            latent[:5].detach().cpu().numpy().astype(np.float32), "...\n")   
        for i in range(num_frames_per_sequence_to_reconstruct):
            filename = get_output_filename(interpolation_dir, "int") + \
                "_seq_" +  str(sequence_id).zfill(3)
            print("Inferring sequence {}, frame {}...".format(sequence_id, i))
            with torch.no_grad():
...
if __name__ == "__main__":
    ...
    arg_parser = argparse.ArgumentParser(description="Infer")

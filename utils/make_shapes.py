...
def create_rotation_operator(angles):
    ...
    """
    if not len(angles) == 3:
        raise ValueError("The input should be a list with 3 angles.")
    else:
        return R.from_euler('xyz', angles)


def init_samples(num_points):
    """"
    ...
    overall_index = np.arange(0, num_points ** 3, 1)
    samples = np.zeros((num_points ** 3, 4))
...
def create_square(center, edge_length, angles, num_points):
    """
    ...
    samples_box_at_center[:, 0:3] = rotation_operation.apply(samples[:, 0:3] - center)

    # Getting the indices that are outside 'box' range (so their absolute value are bigger than half_edge_length)
    num_coord_outside_box = np.sum(np.abs(samples_box_at_center[:, 0:3]) > half_edge_length, axis=1)
...
def check_for_compatible_sdf_samples(samples_1, samples_2):
    ...
    elif not np.linalg.norm(samples_1[:, 0:3] - samples_2[:, 0:3]) == 0:
...
if __name__ == "__main__":
    ...
    # Saving the shape
    directory_name = os.path.join("..", "data", "Squares", "SdfSamples", "Squares", "samples")

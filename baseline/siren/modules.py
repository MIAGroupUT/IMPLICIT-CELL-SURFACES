...
class BatchLinear(nn.Linear, MetaModule):
    ...
    def forward(self, input, params=None):
        ...
        #print('input',input.size()) # ([1, 65536, 3])
        #print('weights',weight.permute(*[i for i in range(len(weight.shape) - 2)], -1, -2).size()) # torch.Size([3, 1024])

        output = input.matmul(weight.permute(*[i for i in range(len(weight.shape) - 2)], -1, -2))
        #print(output.size()) # torch.Size([1, 65536, 1024])
        #exit()

...
def loadMatData(vid_file_path):

    # find all matlab files
    #file_list = [file for file in os.listdir(TRAIN_DATA_DIR) if file.endswith('.mat')]
    
    #file_list.sort()
    
    # limit the amount of data
    #if TRAIN_DATA_FRACTION < 1.0:
    #    file_list = file_list[0:int(TRAIN_DATA_FRACTION * len(file_list))]
    #file_list = file_list[0:1] # first file
    
    #print(file_list)
    
    # convert data to matrices
    #data = np.asarray([sio.loadmat(TRAIN_DATA_DIR + '/' + file)['sdf_vid'] for file in file_list], dtype=np.float32)
    data = np.asarray([sio.loadmat(vid_file_path)['sdf_vid']], dtype=np.float32)  
    #print('shape:',data.shape[:]) # (1, 50, 256, 256, 3)
    
    data = np.squeeze(data)
    
    data = np.expand_dims(data, axis=4)
    #print('shape:',data.shape[:]) # (50, 256, 256, 3, 1)
...
def get_mgrid(sidelen, dim=2):
    ...
    if isinstance(sidelen, int): # is sidelen int?
        sidelen = dim * (sidelen,)
        print('gotcha')
        exit()

    #print('sidelen:',sidelen) #(50, 256, 256, 32)
    ...
    if dim == 2:
        ...
        #print('grid shape', pixel_coords.shape[:]) # (1, 50, 256, 2)
        ...
        #print('grid shape', pixel_coords.shape[:]) # (1, 50, 256, 256, 3)
        
    elif dim == 4: # (t, x, y, z) in the interval [0, 1]
        ...
        #print(pixel_coords[..., 1])
        #print('grid shape', pixel_coords.shape[:]) # (1, 50, 256, 256, 32, 4)
        
    else:
        raise NotImplementedError('Not implemented for dim=%d' % dim)

    print('Grid shape:', pixel_coords.shape[:])
    ...
    #print('shape tensor', pixel_coords.size()) # (1, 50, 256, 256, 32, 4)
...
class Video(Dataset):
    def __init__(self, path_to_video):
        ...
        elif 'mat' in path_to_video:
            self.vid = loadMatData(path_to_video)

        self.shape = self.vid.shape[:-1] # except the last column
        self.channels = self.vid.shape[-1] # only last column
        
        print(self.shape) # cat (300, 512, 512)
        print(self.channels) # cat 3 (rgb)
...
class Implicit3DWrapper(torch.utils.data.Dataset):
    def __init__(self, dataset, sidelength=None, sample_fraction=1., training=False):
        ...
        self.mgrid = get_mgrid(sidelength, dim=4)
        #print('mgrid:', self.mgrid.size())
        #print('sidelength"',sidelength) # (10, 256, 256)
        self.sidelength = sidelength
        #data = (torch.from_numpy(self.dataset[0]) - 0.5) / 0.5 # from [0, 1] to [-1, 1]
        data = torch.from_numpy(self.dataset[0])
        #print('data:',data.size()) # torch.Size([300, 512, 512, 3])
        #print(data.min(),data.max())
        self.data = data.view(-1, self.dataset.channels)
        #print('data size:',data.size())
        self.sample_fraction = sample_fraction
        self.N_samples = int(self.sample_fraction * self.mgrid.shape[0])
        #print('mgrid shape 0:',self.mgrid.shape[0])
        #print('no samples:', self.N_samples)
        #exit()
    ...
    def __getitem__(self, idx):
        if self.sample_fraction < 1.:
            ...
            if self.training:
                ...
                data = self.data[coord_idx, :] # torch.Size([65536, 3])
                #print('data size:',data.size())
                coords = self.mgrid[coord_idx, :]
                #print('coords size:',coords.size())
            else: # TESTING  
                # size of one testing sample, e.g., (256 x 256 x 32)       
                slice_size = self.sidelength[1] * self.sidelength[2] * 16
                #print('slice size:',slice_size)
                # low, high, size
                #coord_idx = torch.randint(0, self.data.shape[0], (self.N_samples,))
                #print(coord_idx)
                
                data = self.data[range(slice_size*self.current_sample, slice_size*(self.current_sample + 1)), :]
                #print('range:', slice_size*self.current_sample, 'to', slice_size*(self.current_sample + 1))
                #print('data size:',data.size())
                #exit()
                coords = self.mgrid[range(slice_size*self.current_sample, slice_size*(self.current_sample + 1)), :]

...
p.add_argument('--num_epochs', type=int, default=3000,
               help='Number of epochs to train for.') # def: 100000
...
p.add_argument('--sample_frac', type=float, default=0.005,
...
coord_dataset = dataio.Implicit3DWrapper(vid_dataset, sidelength=vid_dataset.shape, sample_fraction=opt.sample_frac, training=True)
#exit()
dataloader = DataLoader(coord_dataset, shuffle=True, batch_size=opt.batch_size, pin_memory=True, num_workers=0)

# Define the model.
# If pixel coordinates are 2D, the siren has 2 input features, and if the image is grayscale, it has one output channel (RGB = 3 channels).
